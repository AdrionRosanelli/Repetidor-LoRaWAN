...
def transmit(env,node):
    while node.buffer > 0.0:
        ...
        rssi_base = 0
           ...
           # Verifica primeira repeticao no node 
        if node.nrepet == 0:
            node.packet.repetidor = 1
            
        # verifica e realiza repetição
        if node.packet.repetidor == 1:
            # Potencia adicionada devido repetidor perto do gateway (4dBm)
            rssi_base = (node.packet.txpow + 4) - Lpld0 - 10*gamma*math.log10(node.dist/d0)
        ...
        # adiciona componente aleatória no rssi
