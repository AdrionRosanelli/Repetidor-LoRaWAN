...
def transmit(env,node):
    while node.buffer > 0.0:
        ...
        rssi_base = 0
           ...
           # Verifica se o node esta no alcance (circulo preto) do repetidor e pacote ainda nao foi repetido
        if (node.distrepet <= repet_dist and node.packet.repetidor == 0):
...
if (graphics == 1):
    ...
    ax.add_artist(plt.Circle((repet_x, repet_y),(maxDist/2), fill=False, color='black'))
