...
class myNode():
    def __init__(self, nodeid, bs, period, datasize):
        ...
        #distancia do node para o repetidor
        self.distrepet = np.sqrt((self.x-repet_x)*(self.x-repet_x)+(self.y-repet_y)*(self.y-repet_y))
...
def transmit(env,node):
    while node.buffer > 0.0:
        ...
        # para definir rssi e adicionar a componente aleatoria depois
        rssi_base = 0
       
           # Verifica se o node esta no alcance do repetidor e a distancia ate o gateway
            # é maior que a do repetidor e pacote ainda nao foi repetido
        if (node.distrepet <= repet_dist and repet_dist <= node.dist and node.packet.repetidor == 0):
            node.packet.repetidor = 1
            
        # verifica e realiza repetição
        if node.packet.repetidor == 1:
            # Pacote do repetidor para o gatway
            rssi_base = node.packet.txpow - Lpld0 - 10*gamma*math.log10(repet_dist/d0)
            node.buffer += PcktLength_SF[node.parameters.sf-7]
            node.packet.repetidor = 2
            node.nrepet = node.nrepet + 1
            # modificação de parametros no pacote repetido
            node.packet.sf = random.randint(minsf_repet+1,12)
            node.packet.freq = random.choice([872000000, 864000000, 860000000])
            print ("REPETIDO node {0.nodeid} sf {0.packet.sf} freq {0.packet.freq}".format(node))             
        else:
            # Rssi do node para o gatway (sem repetidor)
            rssi_base =  node.packet.txpow - Lpld0 - 10*gamma*math.log10(node.dist/d0)
            node.packet.repetidor = 0
        
        # subtrai componente aleatória (do path loss) no rssi
        node.packet.rssi = rssi_base - np.random.normal(-var, var)
        ...
        if (node.packet.lost == 0\
            ...
            if((rssi_base - np.random.normal(-var, var)) > sensi[node.packet.sf-7, [125,250,500].index(node.packet.bw) + 1]):
            # the ack is not lost
...
# Posicionamento do repetidor
repet_x = bsx - (maxDist/2)
repet_y = bsy
repet_dist = np.sqrt((repet_x-bsx)*(repet_x-bsx)+(repet_y-bsy)*(repet_y-bsy))

Lpl_repet = Lpld0 + 10*gamma*math.log10(repet_dist/d0) + var
Prx_repet = 14 - GL - Lpl_repet
minairtime_r = 9999
minsf_repet = 0

for i in range(0,6):  # SFs
    if ((sensi[i, [125,250,500].index(Bandwidth) + 1]) < Prx_repet):
        at_r = airtime(i+7, CodingRate , LorawanHeader+PcktLength_SF[i], Bandwidth)
        if at_r < minairtime_r:
            minairtime_r = at_r
            minsf_repet = i+7
            minsensi_r = sensi[i, [125,250,500].index(Bandwidth) + 1]
...
if (graphics == 1):
    ...
    # grafico repetidor
    ax.add_artist(plt.Circle((repet_x, repet_y), 3, fill=True, color='black'))
    #ax.add_artist(plt.Circle((repet_x, repet_y),(maxDist/2), fill=False, color='black',linestyle='--'))
    #ax.add_artist(plt.Circle((bsx, bsy),repet_dist, fill=False, color='black',linestyle='--'))
   
    ax.add_patch(patches.Arc((repet_x, repet_y), 2*repet_dist, 2*repet_dist, 0 , 60, 300, color='black'))
    ax.add_patch(patches.Arc((bsx, bsy), 2*repet_dist, 2*repet_dist, 0, 120, 240, color='black'))
...
der3 = (sent-nrCollisions-nrLost)/float(sent) if sent!=0 else 0
print ("DER 3:", der3)

#numero de repeticoes realizadas
nrepet = sum(n.nrepet for n in nodes)
print ("numero repeticao: ", nrepet)
...
fname = str("Lorawan_v6") + ".dat"
print (fname)
if os.path.isfile(fname):
     res= "\n" + str(sys.argv[5]) + ", " + str(full_collision) + ", " + str(nrNodes) + ", " + str(avgSendTime) + ", " + str(datasize) + ", " + str(sent) + ", "  + str(nrCollisions) + ", "  + str(nrLost) + ", "  + str(nrLostError) + ", " +str(nrNoACK) + ", " +str(nrACKLost) + ", " + str(env.now)+ ", " + str(der1) + ", " + str(der2) + ", " + str(der3)  + ", " + str(energy) + ", "  + str(nodefair1) + ", "  + str(nodefair2) + ", "  + str(SFdistribution)
else:
     res = "#randomseed, collType, nrNodes, TransRate, DataSize, nrTransmissions, nrCollisions, nrlost, nrlosterror, nrnoack, nracklost, CollectionTime, DER1, DER2, DER3, OverallEnergy, nodefair1, nodefair2, sfdistribution\n" + str(sys.argv[5]) + ", " + str(full_collision) + ", " + str(nrNodes) + ", " + str(avgSendTime) + ", " + str(datasize) + ", " + str(sent) + ", "  + str(nrCollisions) + ", "  + str(nrLost) + ", "  + str(nrLostError) + ", " +str(nrNoACK) + ", " +str(nrACKLost) + ", " + str(env.now)+ ", " + str(der1) + ", " + str(der2) + ", " + str(der3)  + ", " + str(energy) + ", "  + str(nodefair1) + ", "  + str(nodefair2) + ", "  + str(SFdistribution)
