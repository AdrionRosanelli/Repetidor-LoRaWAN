...
def retransmit(env,node):
    while node.buffer > 0.0:
        # Potencia adicionada devido repetidor perto do gateway (4dBm)
        node.packet.rssi = (node.packet.txpow + 4) - Lpld0 - 10*gamma*math.log10(node.dist/d0) - np.random.normal(-var, var)
        # add maximum number of retransmissions
        if (node.lstretans and node.lstretans <= 0):
            node.first = 0
            node.buffer += PcktLength_SF[node.parameters.sf-7]
        ...
        if (node.packet.lost == 0\
            ...
            # the packet can be acked
            # check if the ack is lost or not
            if(((node.packet.txpow + 4) - Lpld0 - 10*gamma*math.log10(node.dist/d0) - np.random.normal(-var, var)) > sensi[node.packet.sf-7, [125,250,500].index(node.packet.bw) + 1]):
            # the ack is not lost
                node.packet.acklost = 0
            else:
            # ack is lost
...
for i in range(0,nrNodes):
    ...
    # processo de retransmissao em todos os nós
    node.buffer = datasize 
    env.process(retransmit(env,node))
...
fname = str("confirmablelorawanTest") + ".dat"
print (fname)
